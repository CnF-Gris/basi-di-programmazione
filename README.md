# Introduzione alla programmazione

> [!NOTE]
> Anche se in qualche parte di questo documento troverete delle parti scritte in `c++`,
> noi consigliamo `Python` come linguaggio del corso perché ha una sintassi più
> flessibile e un'enorme quantità di librerie da cui attingere.

## Indice

- Primi passi
  - [Sistemi Numerici Posizionali](./chapters/first-steps/NUMERIC-SYSTEMS.md)
  - [Algoritmi](./chapters/first-steps/ALGORITHMS.md)
  - [Istruzioni di base](./chapters/first-steps/BASIC-INSTRUCTIONS.md)
  - [Tipi](./chapters/first-steps/TYPES.md)
  - [Funzioni](./chapters/first-steps/FUNCTIONS.md)
- Appendice
  - [How-To](./chapters/quality-of-life/HOW-TO.md)
  - [Manuali e Documentazione](./chapters/quality-of-life/MANUALS-AND-DOCS.md)